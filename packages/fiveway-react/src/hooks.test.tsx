...
import { Navnode, useNavnode } from "./node";

test("useFocusedId returns focus path when scope contains focus", async () => {
	const tree = createNavigationTree();

	const App = () => {
		const nav = useNavnode("app", verticalHandler);
...
test("useIsFocused reflects focus inside node", async () => {
	...
	const FocusableItem = (props: { id: string }) => {
		...
		const focused = useIsFocused(nav.id);

		return <div className={focused ? "focused" : ""}>{nav.id}</div>;
	};
...
test("useOnFocusChange reports focus changes", async () => {
	...
	const App = () => {
		const nav = useNavnode("app", verticalHandler);
		useOnFocusChange(nav.id, onFocusChange);
...
test("useOnBlur runs when focus leaves scope", async () => {
	...
	const App = () => {
		const app = useNavnode("app", verticalHandler);
		useOnBlur(app.id, onBlur);
