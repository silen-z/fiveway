import { Nav, useNav, verticalHandler } from "@fiveway/react";
...
export function Items(props: { id: string; order: number; depth: number }) {
	const nav = useNav(props.id, verticalHandler, { order: props.order });
	...
	if (props.depth === 0) {
		return (
			<div>
				<nav.Context>
					{items.map((item) => (
						<Nav key={item.id} id={item.id} order={item.order}>
							{(node) => (
								<div>
